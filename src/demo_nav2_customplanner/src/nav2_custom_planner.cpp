...
#include "demo_nav2_customplanner/nav2_custom_planner.hpp"
...
namespace nav2_custom_planner
{
    ...
    {
        RCLCPP_INFO(
            node_->get_logger(), "Cleaning up planner %s of type Nav2CustomPlanner",
            name_.c_str());
    }
    void CustomPlanner::activate()
    {
        RCLCPP_INFO(
            node_->get_logger(), "Activating planner %s of type Nav2CustomPlanner",
            name_.c_str());
    }
    void CustomPlanner::deactivate()
    {
        RCLCPP_INFO(
            node_->get_logger(), "Deactivating planner %s of type Nav2CustomPlanner",
            name_.c_str());
    }
        ...
        const geometry_msgs::msg::PoseStamped &goal)
    {
        ...
        {
            RCLCPP_ERROR(node_->get_logger(), "规划器仅接受来自%s坐标系的起始位姿", global_frame_.c_str());
            return global_path;
        }
