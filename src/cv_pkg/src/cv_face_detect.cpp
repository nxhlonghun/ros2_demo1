...
void FacePosCallback(const sensor_msgs::msg::RegionOfInterest::SharedPtr msg)
{
    ...
    // int x1=msg->x_offset;
    // int y1=msg->y_offset;
    // int x2=msg->x_offset + msg->width;
    // int y2=msg->y_offset + msg->height;
    // int img_width = imgFace.cols;   // 图像宽度（列数）
    // int img_height = imgFace.rows;
    // float fVelFoward = (img_height/2-(y1+y2)/2)*0.002; // Difference * Proportion
    // float fVelTurn = (img_width/2-(x1+x2)/2)*0.003; 
